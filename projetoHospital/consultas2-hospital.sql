--Quais pacientes realizaram consultas com médicos da especialidade 'Cardiologia' nos últimos 6 meses?
SELECT p.nome AS nome_paciente,
f.nome AS nome_medico,
m.especialidade AS especialidade,
c.data_consulta AS consulta
FROM consulta AS c
JOIN paciente AS p ON (p.id = c.id_paciente)
JOIN medico AS m ON (m.id = c.id_medico)
JOIN funcionario AS f ON (f.id = m.id_funcionario) 
WHERE m.especialidade = 'Cardiologia' 
AND c.data_consulta >= CURRENT_DATE - INTERVAL '6 MONTHS';

--Qual o estoque atual de medicamentos que possuem validade dentro dos próximos 30 dias?
SELECT nome, quantidade_estoque, validade
FROM medicamento  
WHERE validade <= CURRENT_DATE + INTERVAL '30 DAYS';

--Quais médicos trabalharam mais de 40 horas nos últimos 30 dias, somando consultas e plantões?
...
GROUP BY f.nome
HAVING (SUM(c.tempo_consulta) + SUM(p.tempo_plantao)) > INTERVAL '40 HOURS'

--Quantos pacientes foram internados por mais de 7 dias nos últimos 3 meses, e em quais alas do hospital?
...
--Qual foi o medicamento mais utilizado em cirurgias nos últimos 6 meses?
